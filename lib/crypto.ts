...
// Derives a strong AES-KW key from a password and salt using PBKDF2
export async function deriveWrappingKey(password: string, salt: Uint8Array) {
  ...
  return await crypto.subtle.deriveKey(
    // ⬇️ TypeScript fix applied here ⬇️
    { name: "PBKDF2", salt: salt as BufferSource, iterations: 100000, hash: "SHA-256" },
    keyMaterial, { name: "AES-KW", length: 256 }, true, ["wrapKey", "unwrapKey"]
  );
}

// Wraps the RSA private key with the AES wrapping key
export async function wrapPrivateKey(privateKey: CryptoKey, wrappingKey: CryptoKey): Promise<string> {
  const wrapped = await crypto.subtle.wrapKey("pkcs8", privateKey, wrappingKey, { name: "AES-KW" });
